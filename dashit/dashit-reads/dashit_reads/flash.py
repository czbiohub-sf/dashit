...
def poor_structure(forward_kmer, filter_params, need_explanation=False):
    """
    Filter a 20-mer protospacer for poor structure
    """
    ...
    gc_freq_min, gc_freq_max = filter_params['gc_frequency']
    if gc_freq < gc_freq_min or gc_freq > gc_freq_max:
        if not need_explanation:
            return True
        reasons.append('gc_frequency')
    if longest_consecutive_run(kmer) > filter_params['homopolymer']:
        if not need_explanation:
            return True
        reasons.append(f'homopolymer>{filter_params["homopolymer"]}')
    if longest_dinucleotide_run(kmer) > filter_params['dinucleotide_repeats']:
        if not need_explanation:
            return True
        reasons.append(f'dinucleotide_repeats>{filter_params["dinucleotide_repeats"]}')
    min_inner = filter_params['hairpin']['min_inner']
    min_outer = filter_params['hairpin']['min_outer']
    hairpin = find_hairpin(kmer, min_inner, min_outer)
    if hairpin:
        if not need_explanation:
            return True
...
def find_hairpin(kmer, min_inner=3, min_outer=5, hairpin_bounds={}):
    k = 20
    #
