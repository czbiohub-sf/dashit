...
** Output report
We output the designed guides as an HTML document to provide a bit
more friendly way to interpret the data. We use [[http://jinja.pocoo.org/docs/2.10/templates/][a jinja2]] template
together with [[http://getbootstrap.com][bootstrap 4.0]].

#+BEGIN_SRC html :tangle src/templates/report.jinja2
<!DOCTYPE html>
<html lang="en">
<head>
  <title>DASHit-seq automated guide design</title>
...
<body>
  ...
  <header>
    <div class="container">
      <div class="row">
	<div class="col-lg-3">
	  <img src="Biohub_2color.png" width="200px">
	</div>
	<div class="col-lg-9">
	  <h2>DASHit-seq: automated guide design from sequences</h2>
	</div>
      </div>
      <div class="row">
	<div class="col-lg-3 offset-lg-3">
	  <p class="lead">Input sequences:</p>
	</div>
	<div class="col-lg-6">
	  <p class="lead"><pre>{{ input_sequences }}</pre></p>
	</div>
      </div>
      <div class="row">
	<div class="col-lg-3 offset-lg-3">
	  <p class="lead">Off targets:</p>
	</div>
	<div class="col-lg-6">
	  <p class="lead"><pre>{{ off_targets }}</pre></p>
	</div>
      </div>
    </div>
  </header>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-3" style="height: 600px; overflow-y: scroll;">
	<div class="row">
	  <h3>Guide Library</h3>
	</div>
	...
	<table class="table table-striped">
      ...
      <div class="col-lg-8 offset-lg-1" style="height: 600px; overflow-y: scroll;">
	<div class="row">
	  <h3>Filtered sites</h3>
	</div>
	...
	<table class="table table-striped">
	  <thead>
	    <tr>
	      ...
	      <th scope="col">Filtered because...</th>
	    </tr>
	  </thead>
	  <tbody>
	    {% for item in unused_sites %}
	    <tr>
  ...
  <script src="pileup.js"></script>
  <script type="text/javascript">
    var div = document.getElementById('pileup');
    var p = pileup.create(div, {
    range: {contig: 'RN45s', start: {{ reference_start }}, stop: {{ reference_stop }}},
    tracks: [
    {
...
if __name__ == '__main__':
    ...
    if (result_status == pywraplp.Solver.OPTIMAL or
        ...
        output_html = template.render(guides=library,
                                      unused_sites=[(s,
                                                     filtered_sites[s])
                                                    for s in
                                                    filtered_sites],
                                      input_sequences=args.input,
                                      off_targets=args.offtarget,
                                      reference_start=1,
                                      reference_stop=len(input_sequences[0].seq))

        with open('guides.html', 'w') as output_file:
            output_file.write(output_html)
        
        # # Output a BAM file with the CRISPR targets as aligned
        # # sequences for visualization
        header = { 'HD': {'VN':'1.0'}, 'SQ': [{'LN': len(input_sequences[0].seq), 'SN': 'what'}]}
        
        with pysam.AlignmentFile("guides.bam", "wb", header=header) as output_file:
            for i, site in enumerate(library):
                for target in sites_to_targets[site]:
                    a = pysam.AlignedSegment()
                    a.query_name = 'Guide {}'.format(i)
                    a.query_sequence = site
                    #a.flag = 99
                    a.reference_id = 0
                    a.reference_start = target[1][1] - 17
                    #a.reference_start = 0
    ...
    elif result_status == pywraplp.Solve.FEASIBLE:
        print('Feasible sub-optimal gide')
        for site in library:
            print(site)
    else:
